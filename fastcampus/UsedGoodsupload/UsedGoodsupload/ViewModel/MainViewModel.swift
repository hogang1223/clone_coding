...
struct MainViewModel {
    ...
    let detailWriteFormCellModel = DetailWriteFormCellViewModel()
    ...
    init(_ model: MainModel = MainModel()) {
        ...
        let titleMessage = titleTextFieldViewModel.titleText
            .map { $0?.isEmpty ?? true }
            .startWith(true)
            .map { $0 ? ["- 글 제목을 이력해주세요."] : [] }
        ...
        let detailMessage = detailWriteFormCellModel.contentValue
            .map { $0?.isEmpty ?? true }
            .startWith(true)
            .map { $0 ? ["- 내용을 입력해주세요."] : [] }
